...
def make_mock_state_general(overrides=None):
    state = {}
...
def make_mock_state_detect(nbank=2, dlines=None):
    state = {}
...
def make_mock_state_erosion(ship_type="Constant", classes="Bank Type", filters=None, nlevel=2, per_level=None):
    state = {}
    # Basic fields
...
# --- Unit tests ---
def test_build_general_section_fields_and_defaults():
    state = make_mock_state_general()
...
def test_build_detect_section_nbank_lines_dlines(nbank, dlines, expected_dlines):
    state = make_mock_state_detect(nbank=nbank, dlines=dlines)
    exporter = ConfigurationExporter(state)
...
def test_build_erosion_section_ship_params(ship_type, expected_shiptype):
    state = make_mock_state_erosion(ship_type=ship_type)
    exporter = ConfigurationExporter(state)
...
def test_build_erosion_section_classes_flag(classes, expected_flag, expected_banktype):
    state = make_mock_state_erosion(classes=classes)
    exporter = ConfigurationExporter(state)
    exporter._build_erosion_section()
    section = exporter.config["Erosion"]
...
def test_build_erosion_section_filters(filters, expected_keys):
    state = make_mock_state_erosion(filters=filters)
    exporter = ConfigurationExporter(state)
    exporter._build_erosion_section()
    section = exporter.config["Erosion"]
...
def test_build_erosion_section_levels_and_overrides():
    per_level = {
    ...
    state = make_mock_state_erosion(nlevel=2, per_level=per_level)
    exporter = ConfigurationExporter(state)
    exporter._build_erosion_section()
    section = exporter.config["Erosion"]
