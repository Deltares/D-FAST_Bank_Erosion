...
The graphical user interface that you get by running \dfastbe in default mode or by explicitly specifying \keyw{-{}-mode gui} is tested by means of automated GUI tests as described in \autoref{Sec:GuiTests}.
...
\subsection{Graphical user interface tests} \label{Sec:GuiTests}
...
The \keyw{gui} package interacts with the graphical user interface (GUI) by reading from and writing to the
...
produces the expected output files. Because they require a compiled binary, these tests are excluded
from the GitHub Actions run (\keyw{-m "not binaries"}).

On TeamCity the binary tests are the last link of a build chain (configured as code under
...
The test plan describes the (graphical user interface, unit and regression) tests and how they are automated.

\section{Graphical user interface tests}

The automated graphical user interface tests are run with \keyw{pytest-qt}. The results summarize as below.
