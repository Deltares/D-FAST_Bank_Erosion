...
(\keyw{.pre-commit-config.yaml}) that runs locally before each commit and in continuous integration.
The main checks are:

\begin{itemize}
\item \emph{Black} for code formatting (line length 88);
...
\item \emph{bandit}, \emph{gitleaks}, \emph{detect-secrets} and \emph{trufflehog} for security and
secret scanning;
\item \emph{commitizen} to enforce the Conventional Commits message format;
\item \emph{pytest} (unit, doctest and notebook runs) executed as local hooks.
\end{itemize}
