...
\end{symbollist}
%
The value of the Froude number is limited to values less or equal 0.8.
For the coefficient $\alpha_1$ the following values are used depending on the selected ship type, with $T_s$ \unitbrackets{m} the draught of the ships:

\begin{tabular}{lll}
Ship type & Description & $\alpha_1$ \\ \hline
1 & Push barge & 0.5 \\
2 & RHK ship / Motorship & 0.28 $T_s^\text{1.25}$ \\
3 & Towboat & 1.0 \\ \hline
\end{tabular}
...
In \autoref{Fig5.1} the wave height $H_0$ as function of the distance from the fairway $s$ is depicted for various values of the water depth $h$ for a motor ship with a draught of 1.2 m with a relative velocity of $v_s = 6$ m/s.
...
It's assumed that the bank recedes in its entirety and that's why the area of influence \emph{above} the water surface is determined by
%
\begin{equation}
...
\end{equation}
%
where $h_s = \max [ z_\text{bank} - z_w(Q_\text{ref}) - 2 H_0, 0 ]$.
