...
def set_bbox(ax: matplotlib.axes.Axes, bbox: Tuple[float, float, float, float], scale: float = 1000) -> None:
    """
...
def chainage_markers(xykm: numpy.ndarray, ax: matplotlib.axes.Axes, ndec: int = 1, scale: float = 1000) -> None:
    """
    ...
    ax.plot(xykm[mask, 0]/scale, xykm[mask, 1]/scale, linestyle="None", marker="+", color="k")
    for i in numpy.nonzero(mask)[0]:
        ax.text(xykm[i, 0]/scale, xykm[i, 1]/scale, labelstr.format(xykm[i, 2]), fontsize="x-small", clip_on=True)

def plot_mesh(ax: matplotlib.axes.Axes, xe: numpy.ndarray, ye: numpy.ndarray, scale: float = 1000) -> None:
    """
...
def plot_mesh_patches(ax: matplotlib.axes.Axes, fn: numpy.ndarray, nnodes: numpy.ndarray, xn: numpy.ndarray, yn: numpy.ndarray, val: numpy.ndarray, minval: float, maxval: float, scale: float = 1000) -> matplotlib.collections.PolyCollection:
    """
    ...
    p = ax.tripcolor(xn/scale, yn/scale, tfn, facecolors=tval, cmap="viridis_r", vmin=minval, vmax=maxval)
    return p

def plot_detect1(bbox: Tuple[float, float, float, float], xykm: numpy.ndarray, bankareas: List[shapely.geometry.polygon.Polygon], bank: List[shapely.geometry.linestring.LineString], fn: numpy.ndarray, nnodes: numpy.ndarray, xn: numpy.ndarray, yn: numpy.ndarray, h: numpy.ndarray, hmax: float, xlabel_txt: str, ylabel_txt: str, title_txt: str, waterdepth_txt: str) -> matplotlib.figure.Figure:
    """
    ...
    ax.set_aspect(1)
    #
    chainage_markers(xykm, ax, scale=1)
...
def plot1_waterdepth_and_banklines(bbox: Tuple[float, float, float, float], xykm: numpy.ndarray, banklines: geopandas.geodataframe.GeoDataFrame, fn: numpy.ndarray, nnodes: numpy.ndarray, xn: numpy.ndarray, yn: numpy.ndarray, h: numpy.ndarray, hfw_max: float, xlabel_txt: str, ylabel_txt: str, title_txt: str, waterdepth_txt: str) -> matplotlib.figure.Figure:
    """
    ...
    #
    scale = 1000
    chainage_markers(xykm, ax, scale=scale)
...
def plot2_eroded_distance_and_equilibrium(bbox: Tuple[float, float, float, float], xykm: numpy.ndarray, bank_crds: List[numpy.ndarray], dn_tot: List[numpy.ndarray], to_right: List[bool], dnav: numpy.ndarray, xy_eq: List[numpy.ndarray], xe: numpy.ndarray, ye: numpy.ndarray, xlabel_txt: str, ylabel_txt: str, title_txt: str) -> matplotlib.figure.Figure:
    """
    ...
    #
    # plot_mesh(ax, xe, ye)
    chainage_markers(xykm, ax)
    dnav_max = dnav.max()
    for ib in range(len(xy_eq)):
        ax.plot(xy_eq[ib][:,0]/scale, xy_eq[ib][:,1]/scale, linewidth=2, color="k")
        #
        if to_right[ib]:
            bankc = bank_crds[ib]
            dnc = dn_tot[ib]
        ...
        p = ax.tripcolor(x/scale, y/scale, tfn, facecolors=tval, edgecolors="face", linewidth=3, cmap=cmap, vmin=0, vmax=2*dnav_max)
    #
    cbar = fig.colorbar(p, ax=ax, shrink=0.5, drawedges=False, label="eroded distance [m]")
    set_bbox(ax, bbox)
    ax.set_xlabel(xlabel_txt)
    ax.set_ylabel(ylabel_txt)
    ax.grid(True)
...
def plot3_eroded_volume_subdivided_1(km: numpy.ndarray, km_step: float, chainage_txt: str, dv: List[List[numpy.ndarray]], ylabel_txt: str, title_txt: str, qlabel: str) -> matplotlib.figure.Figure:
    """
    ...
    ---------
    km : numpy.ndarray
        Array containing the lower boundaries for the chainage bins.
    km_step : float
        Bin width.
    chainage_txt : str
    ...
    #
    n_levels = len(dv)
    clrs = get_colors("Blues", n_levels)
    for iq in range(n_levels):
        for ib in range(len(dv[iq])):
            if ib==0:
                ...
                dvq = dvq + dv[iq][ib]
        if iq==0:
            ax.bar(km + 0.1*km_step, dvq, width=0.8*km_step, color=clrs[iq], label=qlabel.format(iq=iq+1))
            cumdv = dvq
        else:
            ax.bar(km + 0.1*km_step, dvq, width=0.8*km_step, bottom=cumdv, color=clrs[iq], label=qlabel.format(iq=iq+1))
            cumdv = cumdv + dvq
    #
...
def plot3_eroded_volume_subdivided_2(km: numpy.ndarray, km_step: float, chainage_txt: str, dv: List[List[numpy.ndarray]], ylabel_txt: str, title_txt: str, banklabel: str) -> matplotlib.figure.Figure:
    """
    ...
    clrs = get_colors("YlOrRd", n_banklines)
    for ib in range(n_banklines):
        for iq in range(len(dv)):
            if iq==0:
                ...
                dvq = dvq + dv[iq][ib]
        if ib==0:
            ax.bar(km + 0.1*km_step, dvq, width=0.8*km_step, color=clrs[ib], label=banklabel.format(ib=ib+1))
            cumdv = dvq
        else:
            ax.bar(km + 0.1*km_step, dvq, width=0.8*km_step, bottom=cumdv, color=clrs[ib], label=banklabel.format(ib=ib+1))
            cumdv = cumdv + dvq
    #
    ax.set_xlabel(chainage_txt)
    ax.set_ylabel(ylabel_txt)
...
def plot4_eroded_volume_eq(km: numpy.ndarray, km_step: float, chainage_txt: str, vol_eq: numpy.ndarray, ylabel_txt: str, title_txt: str) -> matplotlib.figure.Figure:
    """
    ...
    setsize(fig)
    #
    tvol = numpy.zeros(km.shape)
    for i in range(len(km)):
        tvol[i] = vol_eq[i].sum()
    ax.bar(km + 0.1*km_step, tvol, width=0.8*km_step)
    #
    ax.set_xlabel(chainage_txt)
    ax.set_ylabel(ylabel_txt)
    ax.grid(True)
...
def plot5series_waterlevels_per_bank(bank_km: List[numpy.ndarray], chainage_txt: str, waterlevel: List[List[numpy.ndarray]], waterlevelq_txt: str, avg_waterlevel_txt:str, bankheight: List[numpy.ndarray], bankheight_txt: str, bankprotect: List[numpy.ndarray], bankprotect_txt: str, elevation_txt: str, title_txt: str, elevation_unit: str) -> List[matplotlib.figure.Figure]:
    """
    ...
    for ib in range(n_banklines):
        ...
        ax.plot(bank_km[ib], wl_avg, color=(0.5, 0.5, 0.5), linewidth=2, label=avg_waterlevel_txt)
        ax.plot(bank_km[ib], bankprotect[ib], color=(0.5, 0.5, 0.5), linestyle="--", label=bankprotect_txt)
        ax.plot(bk, bankheight[ib], color=(0.5, 0.5, 0.5), label=bankheight_txt)
        #
        for iq in range(n_levels):
            ax.plot(bank_km[ib], waterlevel[iq][ib], color=clrs[iq], label=waterlevelq_txt.format(iq=iq+1))
        ax.set_xlabel(chainage_txt)
...
def plot6series_velocity_per_bank(bank_km: List[numpy.ndarray], chainage_txt: str, veloc: List[List[numpy.ndarray]], velocq_txt: str, tauc: List[numpy.ndarray], chezy: List[numpy.ndarray], rho: float, g: float,  ucrit_txt: str, ylabel_txt: str, title_txt: str, veloc_unit: str) -> List[matplotlib.figure.Figure]:
    """
    ...
    for ib in range(n_banklines):
        ...
        for iq in range(n_levels):
            ax.plot(bk, veloc[iq][ib], color=clrs[iq], label=velocq_txt.format(iq=iq+1))
        #
        ax.set_xlabel(chainage_txt)
...
def plot7_banktype(bbox: Tuple[float, float, float, float], xykm: numpy.ndarray, bank_crds: List[numpy.ndarray], banktype: List[numpy.ndarray], taucls_str: List[str], xlabel_txt: str, ylabel_txt: str, title_txt: str) -> matplotlib.figure.Figure:
    """
    ...
    scale = 1000
    chainage_markers(xykm, ax, scale=scale)
    clrs = get_colors("plasma", len(taucls_str))
    for ib in range(len(bank_crds)):
        for ibt in range(len(taucls_str)):
            ...
            if isbanktype.any():
                ...
                if ib == 0:
                    ax.plot(numpy.nan, numpy.nan, color=clrs[ibt], label=taucls_str[ibt])
    #
    set_bbox(ax, bbox)
    ax.set_xlabel(xlabel_txt)
    ax.set_ylabel(ylabel_txt)
...
def plot8_eroded_distance(bank_km: List[numpy.ndarray], chainage_txt: str, dn_tot: List[numpy.ndarray], dn_tot_txt: str, dn_eq: List[numpy.ndarray], dn_eq_txt: str, dn_txt: str, dn_unit: str) -> matplotlib.figure.Figure:
    """
    ...
    for ib in range(n_banklines):
        ...
        ax.plot(bk, dn_eq[ib], linestyle=":", color=clrs[ib], label=dn_eq_txt.format(ib = ib+1))
    #
    ax.set_xlabel(chainage_txt)
